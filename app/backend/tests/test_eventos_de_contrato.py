...
class TestAditivoRenovacaoEncerramento:
    ...
    def test_encerramento_exige_quem_decidiu(self):
        with pytest.raises(ErroDeEvento, match="quem decidiu"):
            efeito_do_evento(C(), pedido(T.ENCERRAMENTO, motivo_categoria=M.PRECO))

    def test_encerramento_exige_a_categoria_do_motivo(self):
        with pytest.raises(ErroDeEvento, match="categoria do motivo"):
            efeito_do_evento(C(), pedido(T.ENCERRAMENTO, iniciativa=I.CLIENTE))
        with pytest.raises(ErroDeEvento, match="categoria do motivo"):
            efeito_do_evento(C(), pedido(T.ENCERRAMENTO, iniciativa=I.CLIENTE, descricao="só texto, sem categoria"))

    def test_encerramento_completo_fecha_o_contrato_sem_exigir_texto(self):
        e = efeito_do_evento(C(), pedido(T.ENCERRAMENTO, iniciativa=I.CRITERIO, motivo_categoria=M.PRECO))
        assert e == {"situacao": S.ENCERRADO, "data_fim": HOJE}

    def test_iniciativa_e_categoria_se_combinam_livremente(self):
        # "Preço" pode ser o cliente que achou caro ou a Critério que reajustou para sair.
        for quem in (I.CLIENTE, I.CRITERIO):
            assert efeito_do_evento(C(), pedido(T.ENCERRAMENTO, iniciativa=quem, motivo_categoria=M.PRECO))

    def test_categoria_outro_exige_texto(self):
        with pytest.raises(ErroDeEvento, match="Outro"):
            efeito_do_evento(C(), pedido(T.ENCERRAMENTO, iniciativa=I.CLIENTE, motivo_categoria=M.OUTRO))
        with pytest.raises(ErroDeEvento, match="Outro"):
